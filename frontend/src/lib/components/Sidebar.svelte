<script lang="ts">
	...
	async function handleItemsDndConsider(
		event: CustomEvent<DndEvent<CollectionItem>>,
		collectionId: string
	) {
		const { items: newItems } = event.detail;
		collectionsStore.update((collections) =>
			collections.map((collection) =>
				collection.id === collectionId ? { ...collection, items: newItems } : collection
			)
		);
	}

	async function handleItemsDndFinalize(
		event: CustomEvent<DndEvent<CollectionItem>>,
		collectionId: string
	) {
		const { items: newItems } = event.detail;
	...
	async function saveCollectionName(id: string, event: Event) {
		...
		if (newName) {
			await collectionsStore.renameCollection(id, newName);
		}
		editingCollectionId = null;
	}
	...
	async function deleteItem(collectionId: string, itemId: string) {
		// Item deletion not implemented for Strapi yet
	}
</script>

<aside class="sidebar">
	<div class="sidebar-header">
		<button class="add-collection-button" onclick={openCreateModal}> Add Collection </button>
	</div>
						...
						>
							<span class="chevron" class:open={$collectionStates.getState(collection.id).isOpen}>
								<button
									...
									aria-label={$collectionStates.getState(collection.id).isOpen
										? 'Collapse section'
										: 'Expand section'}
								>
									<svg
									...
									<div class="item">
										...
										{#if activeDropdown === item.id}
											<div class="dropdown-menu">
												...
												<button onclick={() => deleteItem(collection.id, item.id)}> Delete </button>
											</div>
										{/if}
									</div>
								{/each}
